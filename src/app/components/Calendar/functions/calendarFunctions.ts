...
export const convertADtoBS = (adYear, adMonth?, adDate?) => {
  if (moment.isMoment(adYear) || moment.isDate(adYear)) {
    adYear = moment(adYear);
    adMonth = adYear.month() + 1;
    adDate = adYear.date();
    adYear = adYear.year();
  }
  ...
  return {
    ...
    formatted2: `${calendarData.bsMonths[bsMonth - 1]} ${bsDate}, ${bsYear}`,
    formatted3: `${calendarData.bsMonths[bsMonth - 1]} ${bsDate}`,
  };
};
...
export const bsFullDate = (date: string | Date | moment.Moment) => {
  date = moment(date);
  return convertADtoBS(date.year(), date.month() + 1, date.date()).formatted2;
};

export const adFullDate = (date: string | Date | moment.Moment) => {
  date = moment(date);
  return date.format('dddd MMM YY');
};
